  ...
  try {
    ...
    const byAgeCohort = await Promise.all(
      AGE_COHORTS.map(async (cohort) => {
        ...
        const visited = await Person.countDocuments({
          age: { $gte: cohort.min, $lte: cohort.max },
          isVisited: true,
        });
        return {
          label: cohort.label,
          total,
          visited,
          rate: total === 0 ? 0 : Math.round((visited / total) * 100),
        };
      })
    );
    ...
    // Flag any group more than 20 points below the overall rate, so the
    // dashboard can highlight where coverage is falling behind.
    const flagThreshold = 20;
    const flagged = (rows) =>
      rows.map((r) => ({ ...r, flagged: r.total > 0 && overallRate - r.rate >= flagThreshold }));
